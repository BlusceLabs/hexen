        ...
        match name:
            ...
            case "_page":
                if not isinstance(value, int):
                    raise TypeError(
                        f"Expected int for '_page', got {type(value).__name__}"
                    )
            ...
            case "_genre_top_id":
                if not validate_genre_top_id(value):
                    raise ValueError(
...
class SearchSuggestion(hexenapi.v1.core.SearchSuggestion):
    _url = get_absolute_url("/wefeed-h5api-bff/subject/search-suggest")


class Search(hexenapi.v1.core.Search):
