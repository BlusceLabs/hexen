...
async def fetch_tmdb(path: str, params: dict) -> dict:
    from hexenapi.backend.tmdb import (
        TMDBClient, Movies, TV, People, Search, Discover, Trending,
        Genres, Collections, Companies, Keywords, Find, WatchProviders, Configuration,
    )
...
async def _handle_movie(c, parts, params):
    ...
    if len(parts) == 1:
        ...
        if action == "popular": return {"results": await m.popular(page)}
        if action == "top_rated": return {"results": await m.top_rated(page)}
        if action == "now_playing": return {"results": await m.now_playing(page)}
        if action == "upcoming": return {"results": await m.upcoming(page)}
        if action == "latest": return await m.latest()
        return {"error": f"Unknown action: {action}"}
    mid = int(parts[1])
    if len(parts) == 2:
        return await m.details(mid)
    sub = parts[2]
    if sub == "credits": return {"cast": await m.credits(mid)}
    if sub == "videos": return {"videos": await m.videos(mid)}
    if sub == "images": return await m.images(mid)
    if sub == "logos": return {"logos": await m.logos(mid)}
    if sub == "reviews": return {"reviews": await m.reviews(mid)}
    if sub == "translations": return {"translations": await m.translations(mid)}
    if sub == "release_dates": return {"release_dates": await m.release_dates(mid)}
    if sub == "watch_providers": return {"watch_providers": await m.watch_providers(mid)}
    if sub == "similar": return {"results": await m.similar(mid)}
    if sub == "recommendations": return {"results": await m.recommendations(mid)}
    if sub == "keywords": return {"keywords": await m.keywords(mid)}
    if sub == "alternative_titles": return {"titles": await m.alternative_titles(mid)}
    if sub == "external_ids": return await m.external_ids(mid)
    if sub == "lists": return {"lists": await m.lists(mid)}
    if sub == "changes": return {"changes": await m.changes(mid)}
...
async def _handle_tv(c, parts, params):
    ...
    if len(parts) == 1:
        action = params.get("action", ["popular"])[0]
        page = int(params.get("page", ["1"])[0])
        if action == "popular": return {"results": await tv.popular(page)}
        if action == "top_rated": return {"results": await tv.top_rated(page)}
        if action == "airing_today": return {"results": await tv.airing_today(page)}
        if action == "on_the_air": return {"results": await tv.on_the_air(page)}
        if action == "latest": return await tv.latest()
        return {"error": f"Unknown action: {action}"}
    tid = int(parts[1])
    if len(parts) == 2:
        return await tv.details(tid)
    sub = parts[2]
    if sub == "credits": return {"cast": await tv.credits(tid)}
    if sub == "videos": return {"videos": await tv.videos(tid)}
    if sub == "images": return await tv.images(tid)
    if sub == "logos": return {"logos": await tv.logos(tid)}
    if sub == "reviews": return {"reviews": await tv.reviews(tid)}
    if sub == "translations": return {"translations": await tv.translations(tid)}
    if sub == "watch_providers": return {"watch_providers": await tv.watch_providers(tid)}
    if sub == "similar": return {"results": await tv.similar(tid)}
    if sub == "recommendations": return {"results": await tv.recommendations(tid)}
    if sub == "keywords": return {"keywords": await tv.keywords(tid)}
    if sub == "season":
        if len(parts) < 4:
...
async def _handle_person(c, parts, params):
    ...
    pid = int(parts[1])
    if len(parts) == 2: return await p.details(pid)
    sub = parts[2]
    if sub == "movie_credits": return {"movies": await p.movie_credits(pid)}
    if sub == "tv_credits": return {"tv": await p.tv_credits(pid)}
    if sub == "images": return await p.images(pid)
    if sub == "external_ids": return await p.external_ids(pid)
    if sub == "changes": return {"changes": await p.changes(pid)}
    return {"error": f"Unknown sub-resource: {sub}"}
...
async def _handle_search(c, parts, params):
    ...
    if target == "multi": return {"results": await s.multi(query, page)}
    if target == "movie": return {"results": await s.movie(query, page)}
    if target == "tv": return {"results": await s.tv(query, page)}
    if target == "person": return {"results": await s.person(query, page)}
    if target == "collection": return {"results": await s.collection(query, page)}
    if target == "company": return {"results": await s.company(query, page)}
    if target == "keyword": return {"results": await s.keyword(query, page)}
...
async def _handle_discover(c, parts, params):
    ...
    if target == "movie": return {"results": await d.movies(**filters)}
    if target == "tv": return {"results": await d.tv(**filters)}
...
async def _handle_trending(c, parts, params):
    ...
    if target == "all": return {"results": await t.all(tw)}
    if target == "movie": return {"results": await t.movies(tw)}
    if target == "tv": return {"results": await t.tv(tw)}
    if target == "person": return {"results": await t.people(tw)}
...
async def _handle_genre(c, parts):
    ...
    g = Genres(c)
    target = parts[1] if len(parts) > 1 else "movie"
    if target == "movie": return {"genres": await g.movie_list()}
    if target == "tv": return {"genres": await g.tv_list()}
...
async def _handle_collection(c, parts):
    ...
    col = Collections(c)
    cid = int(parts[1])
    if len(parts) == 2: return await col.details(cid)
    if len(parts) > 2 and parts[2] == "images": return await col.images(cid)
...
async def _handle_company(c, parts):
    ...
    comp = Companies(c)
    cid = int(parts[1])
    if len(parts) == 2: return await comp.details(cid)
    if len(parts) > 2 and parts[2] == "images": return await comp.images(cid)
    if len(parts) > 2 and parts[2] == "alternative_names": return {"names": await comp.alternative_names(cid)}
    return {"error": f"Unknown sub-resource: {parts[2]}"}
...
async def _handle_keyword(c, parts):
    ...
    kw = Keywords(c)
    kid = int(parts[1])
    if len(parts) == 2: return await kw.details(kid)
    if len(parts) > 2 and parts[2] == "movies": return {"results": await kw.movies(kid)}
    if len(parts) > 2 and parts[2] == "tv": return {"results": await kw.tv(kid)}
    return {"error": f"Unknown sub-resource: {parts[2]}"}
...
async def _handle_watch(c, parts):
    ...
    if target == "regions": return {"regions": await wp.regions()}
    if target == "movie": return {"results": await wp.movie()}
    if target == "tv": return {"results": await wp.tv()}
    ...
    if action == "info":
        ...
        try:
            ...
            return {
                ...
                "downloads": sorted(downloads, key=lambda x: x["resolution"], reverse=True),
            }
        except Exception as e:
            return {
                "subject_id": subject_id,
        ...
        try:
            ...
            files_model = await detail.get_content_model(season=season, episode=episode)
            downloads = []
            for d in files_model.downloads:
                downloads.append({
                    "url": str(d.url),
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=30) as client:
            ...
            if resp.status_code == 200:
                return {"url": str(resp.url), "status": resp.status_code, "content_type": resp.headers.get("content-type", ""), "content_length": len(resp.content)}
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=60) as client:
            ...
            async with client.stream("GET", url, headers=req_headers) as resp:
                ...
                handler.send_header("Content-Type", resp.headers.get("content-type", "video/mp4"))
...
def main():
    ...
    log.info("Endpoints: /movie, /tv, /person, /search, /discover, /trending, /genre, /collection, /company, /keyword, /find, /watch, /configuration, /health")
    try:
